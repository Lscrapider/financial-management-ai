...
public class AiChatMessageManage extends ServiceImpl<AiChatMessageMapper, AiChatMessagePO> {
    ...
    public long countUserMessagesBetween(Long userId, LocalDateTime startTime, LocalDateTime endTime) {
        return this.lambdaQuery()
                .eq(AiChatMessagePO::getUserId, userId)
                .eq(AiChatMessagePO::getRole, AiChatMessagePO.ROLE_USER)
                .ge(AiChatMessagePO::getCreatedAt, startTime)
                .lt(AiChatMessagePO::getCreatedAt, endTime)
                .count();
    }
}
