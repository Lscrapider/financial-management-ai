...
class ChunkRuleTagger:
    ...
    def _score_rule(self, text: str, rule: TagRule) -> float:
        ...
        return self._score_keywords(text, rule.keyword_patterns)
    ...
    def _build_asset_rules(self) -> tuple[TagRule, ...]:
        return (
            ...
            self._tag("asset", "stock", phrases=(("个股走势", 0.86), ("低价股", 0.86), ("买股", 0.82), ("卖股", 0.82)), keywords=("股票", "个股", "股价", "正股")),
            ...
            self._tag("asset", "low_price_stock", phrases=(("低价股", 0.88), ("几元股", 0.86), ("低价小票", 0.86), ("低价补涨", 0.84))),
    ...
    def _build_price_rules(self) -> tuple[TagRule, ...]:
        return (
            self._tag("price", "price_rise", phrases=(("涨幅扩大", 0.82), ("放量上涨", 0.78), ("价格上涨", 0.75)), keywords=("上涨", "拉升", "冲高", "涨幅")),
            self._tag("price", "price_drop", phrases=(("跌幅扩大", 0.82), ("下跌后的处理", 0.80), ("价格下跌", 0.75)), keywords=("下跌", "杀跌", "回落", "跌幅")),
            ...
            self._tag("price", "breakout", phrases=(("突破压力位", 0.88), ("突破平台", 0.86), ("突破前高", 0.86), ("突破后的确认", 0.84)), keywords=("突破", "站上")),
            self._tag("price", "pullback", phrases=(("回调买点", 0.84), ("上涨后的回调", 0.82), ("短线回落", 0.78)), keywords=("回调", "回踩", "调整")),
    ...
    def _build_volume_rules(self) -> tuple[TagRule, ...]:
        return (
            self._tag("volume", "volume_expand", numeric=((r"量比\s*([0-9]+(?:\.[0-9]+)?)", lambda value: 0.5 + value / 10),), phrases=(("成交量放大", 0.78), ("量能增加", 0.76), ("资金明显活跃", 0.76), ("放量上涨", 0.75)), keywords=("放量", "量能放大")),
            self._tag("volume", "volume_shrink", phrases=(("成交量减少", 0.78), ("量能不足", 0.76), ("无人关注", 0.72)), keywords=("缩量",)),
            self._tag("volume", "high_turnover", numeric=((r"换手率?\s*([0-9]+(?:\.[0-9]+)?)\s*%", lambda value: 0.3 + value / 30),), phrases=(("换手率高", 0.78), ("筹码交换剧烈", 0.78), ("交易活跃", 0.74), ("分歧加大", 0.72)), keywords=("高换手",)),
            ...
            self._tag("volume", "volume_price_confirm", phrases=(("上涨有量", 0.84), ("下跌缩量", 0.82), ("量价配合", 0.82), ("价格和成交量互相验证", 0.82))),
    ...
    def _build_trend_rules(self) -> tuple[TagRule, ...]:
        return (
            self._tag("trend", "uptrend", phrases=(("连续上涨", 0.82), ("趋势向上", 0.82), ("均线多头", 0.80), ("逐步抬高", 0.78), ("多头排列", 0.78))),
            ...
            self._tag("trend", "breakout_from_range", phrases=(("长期横盘后突破", 0.88), ("箱体突破", 0.86), ("平台突破", 0.86), ("横盘突破", 0.84), ("继续放量并站稳", 0.78))),
            self._tag("trend", "failed_breakout", phrases=(("突破后回落", 0.86), ("冲高失败", 0.84), ("突破失败", 0.84), ("站不上去", 0.82), ("假突破", 0.82))),
        )
    ...
    def _build_valuation_rules(self) -> tuple[TagRule, ...]:
        return (
            ...
            self._tag("valuation", "valuation_trap", phrases=(("低估值不一定安全", 0.88), ("便宜有原因", 0.84), ("低估值陷阱", 0.84), ("低 PE 陷阱", 0.84), ("低 PB 陷阱", 0.84))),
    ...
    def _build_sentiment_rules(self) -> tuple[TagRule, ...]:
        return (
            ...
            self._tag("sentiment", "short_term_emotion", phrases=(("短线情绪", 0.82), ("短线资金活跃", 0.82), ("情绪推动上涨", 0.82), ("突然放量上涨", 0.80)), keywords=("短线", "情绪")),
            ...
            self._tag("sentiment", "weak_sentiment", phrases=(("市场情绪弱", 0.82), ("没人接力", 0.80), ("上涨无力", 0.78), ("关注度低", 0.76))),
        )

    def _build_risk_strategy_rules(self) -> tuple[TagRule, ...]:
        return (
            self._tag("risk_strategy", "chase_high_risk", phrases=(("不要盲目追", 0.88), ("不要追高", 0.88), ("接近高位要谨慎", 0.86), ("涨多了风险变大", 0.84), ("追高风险", 0.84)), keywords=("追高",), negation_sensitive=False),
            ...
            self._tag("risk_strategy", "position_control", phrases=(("控制仓位", 0.82), ("不要满仓", 0.82), ("仓位管理", 0.80), ("分批买入", 0.78), ("轻仓", 0.76)), keywords=("分批",), negation_sensitive=False),
            self._tag("risk_strategy", "wait_confirm", phrases=(("不要马上判断", 0.84), ("等待确认", 0.84), ("等确认", 0.82), ("等站稳", 0.82), ("走势验证", 0.80), ("继续放量并站稳", 0.80)), keywords=("站稳",), negation_sensitive=False),
            self._tag("risk_strategy", "observe_next_day", phrases=(("第二天观察", 0.86), ("第二天是否继续放量", 0.86), ("第二天是否站稳", 0.86), ("次日是否继续放量", 0.86), ("次日是否站稳", 0.86), ("明天观察", 0.82)), keywords=("第二天", "次日"), negation_sensitive=False),
            self._tag("risk_strategy", "avoid_emotional_trade", phrases=(("不要冲动", 0.84), ("不要被情绪影响", 0.84), ("不要因为涨跌而乱操作", 0.84), ("避免情绪交易", 0.82)), negation_sensitive=False),
            self._tag("risk_strategy", "take_profit_plan", phrases=(("涨到目标后减仓", 0.84), ("分批卖出", 0.82), ("落袋为安", 0.80)), keywords=("止盈",)),
            self._tag("risk_strategy", "stop_loss_plan", numeric=((r"止损\s*([0-9]+(?:\.[0-9]+)?)\s*%", lambda value: 0.5 + value / 30),), phrases=(("跌破条件止损", 0.84), ("亏损控制", 0.82), ("设置退出条件", 0.80)), keywords=("止损",)),
        )
