...
你需要从以下 7 个大类中选择合适的标签。每个大类最多选 3 个标签，不相关的大类返回空数组。
...
class ChunkLlmTagHandler(MessageHandler):
    ...
    def _parse_and_validate(self, content: str) -> dict[str, list[str]]:
        ...
        for category in SCENE_CATEGORIES:
            ...
            filtered = [t for t in tags if isinstance(t, str) and t in valid_tags]
            result[category] = filtered[:3]
        return result
