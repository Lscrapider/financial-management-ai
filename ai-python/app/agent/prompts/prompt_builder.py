...
class AgentPromptBuilder:
    def system_prompt(self) -> str:
        return (
            ...
            "短期记忆不是可调用工具；如果当前问题需要历史指代或延续上一轮任务，系统会在工具规划前注入短期记忆上下文。"
            "全市场推荐、从全部股票筛选、重新分析、换一个方向，或当前问题已明确股票代码、名称或新标的时，不要因为短期记忆里的旧标的自动延续。"
            "工具选择规则："
            ...
            "调用 scene_signal_context 时必须同时传入标准 target_code 和 target_name；"
            "如果当前只知道证券名称或只知道代码，先调用 market_quote 解析标准代码和名称。"
