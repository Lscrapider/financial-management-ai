...
`klineTrends.daily`、`klineTrends.weekly`、`klineTrends.monthly` 分周期进入报告上下文，不再只依赖日 K。配置窗口按周期自然单位理解，例如月 K 配置 320 表示约 320 个月。
...
- 不同周期窗口是否需要按 reportType / targetType 进一步差异化。
...
- 可转债基础资料、每日估值、份额变化已经接入并落表，报告计算已读取 `assetSpecificData.convertibleBond`。
- 后续重点是补全完整票息现金流 YTM，而不是继续使用当前不含票息的保守近似。
- 强赎 / 回售状态目前有结构占位，后续需要接入公告或条款进度数据，计算真实触发进度。
- 继续优化可转债差异化展示结构和正股联动分析。
