# AI Chat Agent 系统架构与 LangGraph 设计

本文档描述 AI Chat Agent 化后的系统边界、端到端链路、Python LangGraph 编排、Tool Calling、RabbitMQ、HTTP、WebSocket 与内部签名逻辑。
...
LangGraph 只负责 Python Agent 内部执行流转。它把“是否加载上下文”“是否调用工具”“证据是否足够”“最终流式输出”拆成明确节点，避免 planner、工具调用和最终回答互相污染。

### 节点与路由图

```mermaid
flowchart TD
  A["context_gate<br/>规则判断画像/记忆，未确定项用 LLM 兜底"] -->|需要画像| B["load_profile"]
  A -->|不需要画像但需要记忆| C["load_memory"]
  A -->|都不需要| D["planner"]
  B -->|需要记忆| C
  B -->|不需要记忆| D
  C --> D
  D -->|tool_calls| E["tools"]
  D -->|no tool_calls| F["final_decision"]
  E -->|预算允许| D
  E -->|预算/失败/超时| F
  F -->|need_tool 且预算允许| D
  F -->|ready / insufficient| G["final_stream"]
  G --> H["END"]
```

### Graph State 数据契约

Graph State 保存节点之间共享的最小必要状态：

- `messages`：原始用户问题和会话输入。
- `profile_required`、`psych_profile`：是否需要用户画像，以及画像摘要和建议强度。
- `memory_required`、`memory_mode`、`memory_context`：是否需要短期记忆、记忆模式和读取到的上下文。
...
Graph Deps 保存运行依赖，例如模型、工具表、答案生成器、工具执行器、画像 provider、记忆 provider、Token 统计器和流式 delta callback。State 是流程数据，Deps 是外部能力，两者分开，避免节点隐式访问全局状态。

### 节点职责

- `context_gate`：不是工具节点。它先用规则分别判断是否需要用户画像和短期记忆，规则无法判断的项再调用一次不绑定工具的 LLM JSON 兜底；兜底失败时默认加载画像和记忆。
- `load_profile`：通过 Java 数据网关读取 `investor.psych_profile`。如果用户没有画像、读取失败或 provider 不存在，使用默认画像策略：不生成明确买卖建议。
- `load_memory`：通过 Java 数据网关读取 `conversation.history`。短期记忆只作为 planner/final 的上下文，不暴露成 LangChain tool。
...
Python 侧所有 LLM 调用都通过 Token 统计器上报，Java 侧只需要在已有 Token 记录的 `phrase` 字段接受更多阶段值，不改变其他接口设计。

```text
context_gate:
  context_gate 规则无法判断画像或记忆需求时，LLM JSON 兜底分类

initial_planning:
  首轮 planner 调用；如果直接回答，也沿用这个阶段记录 Token

tool_followup_planning:
  tools 返回后，planner 判断是否继续调用工具
