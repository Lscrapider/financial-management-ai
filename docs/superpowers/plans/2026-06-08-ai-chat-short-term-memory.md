...
**Goal:** 为 AI Chat 增加基于 `userId + conversationId` 的短期记忆、WebSocket 关闭后的延迟清理、短期消息摘要入长期记忆，以及 Python 固定召回最近 20 条短期记忆。

**Architecture:** Java 负责 conversation/message/memory 持久化、WebSocket 绑定、后端生成或复用 conversationId、cleanupVersion 防误删和内部数据网关；Python 负责在 Agent Run 开始时读取最近 20 条 `conversation.history` 并放入 LLM 上下文。WebSocket 关闭后只在 activeCount 降为 0 时发送 30 分钟延迟 cleanup，消费时版本一致才先保存 `conversation_summary` 长期记忆，再删除短期消息。
...
- [ ] 握手阶段由 Java 按 userId 自动生成或复用 active conversation。
- [ ] 内存维护 `activeCount`。
...
- [ ] 前端建立 WS 时只带登录 token，不生成 `conversationId`。
- [ ] Java 握手解析用户并绑定后端生成或复用的 conversation。
- [ ] 收消息时保存 user message。
...
- [ ] Python Agent Run 开始时固定读取最近 20 条消息。
